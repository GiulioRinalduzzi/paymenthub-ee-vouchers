...
public class SendErrorWorker {
    ...
    public void setup() {
        zeebeClient.newWorker().jobType("send-failure-voucher").handler((client, job) -> {
            ...
            try {
                Voucher voucher = voucherRepository.findBySerialNo(existingVariables.get("voucherSerialNumber").toString()).orElseThrow(
                        () -> VoucherNotFoundException.voucherNotFound(existingVariables.get("voucherSerialNumber").toString()));
